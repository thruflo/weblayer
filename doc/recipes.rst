...
:ref:`weblayer` is `WSGI`_ compatible and so can be served using `Apache mod_wsgi`_, as shown by `./src/weblayer/examples/mod_wsgi`_.  The application script file is `app.wsgi`:

.. literalinclude:: ../src/weblayer/examples/mod_wsgi/app.wsgi
   :lines: 20-

By default, `Apache mod_wsgi`_ looks for a callable entry point called `application` in the script file.  This is then hooked up in your apache configuration using the `WSGIScriptAlias` directive, e.g.::
  
      WSGIScriptAlias / /path/to/app.wsgi
  
See `this page`_ for a configuration guide explaining how to setup `Apache mod_wsgi`_.

Appengine
---------

As a pure `Python`_ package that's compatible with Python 2.5, :ref:`weblayer` works on `Google App Engine`_, as shown by `./src/weblayer/examples/appengine`_.  The application is in `app.py`:
...
:ref:`weblayer` is `WSGI`_ compatible and so can be served using `Paste`_, as shown by `./src/weblayer/examples/paste`_.  The demo application is in `demo.py`:
...
.. _`./src/weblayer/examples/paste`: http://github.com/thruflo/weblayer/tree/master/src/weblayer/examples/paste
.. _`./src/weblayer/examples/appengine`: http://github.com/thruflo/weblayer/tree/master/src/weblayer/examples/appengine
.. _`./src/weblayer/examples/mod_wsgi`: http://github.com/thruflo/weblayer/tree/master/src/weblayer/examples/mod_wsgi
